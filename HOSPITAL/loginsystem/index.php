...
if (isset($_POST['signup'])) {
	...
	$sql = mysqli_query($con, "select id from tblhospital_user where email='$email'");
	$row = mysqli_num_rows($sql);
	if ($row > 0) {
		echo "<script>alert('Email id already exist with another account. Please try with other email id');</script>";
	} else {
		$msg = mysqli_query($con, "insert into tblhospital_user(`fname`, `lname`, `email`, `password`, `hospital_name`, `hospital_id`, `contactno`) values('$fname','$lname','$email','$enc_password','$hname','$hosid','$contact')");
...
if (isset($_POST['login'])) {
	...
	$ret = mysqli_query($con, "SELECT * FROM tblhospital_user WHERE email='$useremail' and password='$dec_password'");
...
if (isset($_POST['send'])) {
	...
	$row1 = mysqli_query($con, "select email,password from tblhospital_user where email='$femail'");
...
<body>
	<div class="main">
		<h1 style="color:#214ccc;  font-family: 'Raleway', sans-serif;
">HOSPITAL PANEL</h1>
		<div class="sap_tabs">
						...
						<div class="facts">
							...
							<div class="register">
								<form name="registration" method="post" action="" enctype="multipart/form-data">
									...
									<p>Hospital Name </p>
									<input type="text" class="text" value="" name="hname" required>
									<p>Hospital ID Number </p>
									<input type="text" class="text" value="" name="hosid" required>
